...
class WelcomeBot(commands.Cog):
    ...
    async def setup(self,ctx):
        ...
        await ctx.defer()
        welembed = func.Embed().color(0x11111b).title("Welcome Listener Setup").description("Welcome to the Welcome Listener Setup!")
        emb = await ctx.send(embed = welembed.embed)
        await server_config.create_default_server_config(ctx.guild.id)
        welembed.description("Please reply to this message with the channel ID of the welcome channel.")
    ...
    async def joinmsg(self,member):
        ...
        await member.guild.get_channel_or_thread(welid["welcome_channel_id"]).send(func.Embed().title(f"Welcome to {member.guild.name}!").description(f"Welcome {member.mention}!").thumbnail(member.avatar.url).embed)
        
    @commands.Cog.listener()
    async def on_message(self, message:discord.Message):
        if message.reference is not None and message.reference.message_id in self.pending_id.keys():
            ...
            print("waiter not found, ignoring message")
