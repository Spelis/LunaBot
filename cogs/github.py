...
class GitHub(commands.Cog):
    ...
    async def ghuserinfo(self,ctx,username):
        ...
        j = r.json()
        dt = datetime.strptime(j['created_at'],r"%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
    ...
    async def ghrepoinfo(self,ctx,user,repo):
        ...
        j = r.json()
        created_at = datetime.now(timezone.utc) - datetime.strptime(j['created_at'],r"%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        updated_at = datetime.now(timezone.utc) - datetime.strptime(j['updated_at'],r"%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        pushed_at = datetime.now(timezone.utc) - datetime.strptime(j['pushed_at'],r"%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        await ctx.send(
            embed=func.Embed()
    ...
    async def ghrepolangchart(self,ctx,user,repo):
        ...
        total = sum(j.values())
        other = sum(value for value in j.values() if (value/total)*100 < .1)
        j = {k: v for k, v in j.items() if (v/total)*100 >= .1}
        if other > 0:
            j['Other'] = other
